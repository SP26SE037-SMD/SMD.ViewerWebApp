...
export default function LearningPathContent() {
  ...
  useEffect(() => {
    const fetchLearningPath = async () => {
      ...
      try {
        const [requirementsRes, subjectRes] = await Promise.all([
          subjectApiRequest.getPrerequisiteRequirementsByCode(subjectCodeQuery),
        ...
        const requirementRows = requirementsRes?.payload?.data || [];
        setRequirements(requirementRows);
        setSubject(subjectRes?.payload?.data?.content?.[0] || null);
      } catch (error) {
        console.error("Failed to fetch learning path prerequisites", error);
        setSubject(null);
        setRequirements([]);
      } finally {
        setLoading(false);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(query ? `/learning-path?${query}` : "/learning-path");
  };
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              <p className="text-sm text-gray-400">Loading prerequisites...</p>
            </motion.div>
          ) : subjectCodeQuery ? (
            <motion.div
              key="results"
                  ...
                  requirements.map((item) => (
                    ...
                    >
                      <p className="font-semibold">
                        Subject Code: {item.prerequisiteSubjectCode}
                      </p>
                      <p> Subject Name: {item.prerequisiteSubjectName}</p>
                    </div>
                  ))
                ) : (
                  <div className="px-6 py-6 text-sm text-gray-500">
                    No prerequisite subjects found.
                  </div>
                )}
              </div>
            </motion.div>
            ...
            >
              Search by subject code to view prerequisite subjects.
            </motion.div>
          )}
        </AnimatePresence>
      </div>
